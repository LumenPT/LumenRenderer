...
)
{
    ...
    for (int i = index; i < a_NumIntersections; i += stride)
    {
        ...
        float pdf = 0.f;
        bool specular = false;
        const auto bsdf = SampleBSDF(surfaceData.m_ShadingData, surfaceData.m_Normal, surfaceData.m_Normal, surfaceData.m_Tangent, invViewDir, 1.f, RandomFloat(seed), RandomFloat(seed), RandomFloat(seed), bounceDirection, pdf, specular);
