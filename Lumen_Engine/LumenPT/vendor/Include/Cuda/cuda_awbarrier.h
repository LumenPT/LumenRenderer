...
{
    return _CUDA_AWBARRIER_INTERNAL_NAMESPACE::awbarrier_token_pending_count(this->token);
}
...
void init(awbarrier* barrier, uint32_t expected_count)
{
    ...
    _CUDA_AWBARRIER_INTERNAL_NAMESPACE::awbarrier_init(&barrier->barrier, expected_count);
}
...
awbarrier::arrival_token awbarrier::arrive()
{
    ...
    const uint64_t token = _CUDA_AWBARRIER_INTERNAL_NAMESPACE::awbarrier_arrive_drop<false>(&this->barrier);
...
awbarrier::arrival_token awbarrier::arrive_and_drop()
{
    ...
    const uint64_t token = _CUDA_AWBARRIER_INTERNAL_NAMESPACE::awbarrier_arrive_drop<true>(&this->barrier);
