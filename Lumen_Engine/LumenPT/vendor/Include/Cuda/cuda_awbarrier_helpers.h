...
void awbarrier_init(uint64_t* barrier, uint32_t expected_count)
{
    ...
    ImplementationChooser<_CUDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_init(barrier, init_count);
}
...
uint32_t awbarrier_token_pending_count(uint64_t token)
{
    const uint32_t pending_count = ImplementationChooser<_CUDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_token_pending_count(token);
...
uint64_t awbarrier_arrive_drop(uint64_t* barrier)
{
    ...
    const uint64_t token = ImplementationChooser<_CUDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_arrive_drop_no_complete<Drop>(barrier, arrive_count);

    (void)ImplementationChooser<_CUDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_arrive_drop<Drop>(barrier);
